...
NetworkModule::NetworkModule( void ):MonitorModule(17, 6, "RAM USAGE")
{

}

NetworkModule::NetworkModule( string name ):MonitorModule(17, 6, name)
{
...
void					NetworkModule::drawContent( int posX, int posY, int width, int height, Window const & win )
{
	...
	byte_i << "I BYTE: " << byte_i_data ;
	pckt_o << "O PCKT: " << pckt_o_data ;
	byte_o << "O BYTE: " << byte_o_data ;
	...
	int xoffset[4];
	xoffset[0] = (maxWidth - pckt_i.str().size()) / 2;
	xoffset[1] = (maxWidth - byte_i.str().size()) / 2;
	xoffset[2] = (maxWidth - pckt_o.str().size()) / 2;
	xoffset[3] = (maxWidth - byte_o.str().size()) / 2;
